...
export async function generateMetadata({ params }: DishPageProps): Promise<Metadata> {
  try {
    const dish = await pairDishAPI.getDish(params.dish);
    ...
    return generateSEOMetadata({
      ...
      openGraph: {
        ...
        url: `${process.env.NEXT_PUBLIC_SITE_URL}/what-to-serve-with/${params.dish}`,
        image: dish.imageUrl,
      },
    });
  } catch (error) {
...
export default async function DishPage({ params }: DishPageProps) {
  ...
  try {
    [dish, pairings] = await Promise.all([
      pairDishAPI.getDish(params.dish),
      pairDishAPI.getDishPairings(params.dish),
    ]);
  } catch (error) {
    notFound();
  }
