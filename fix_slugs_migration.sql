-- Migration to fix dish slugs by removing "what-to-serve-with-" prefix
-- This will prevent double-prefixing when frontend constructs URLs

-- First, let's see what slugs we currently have
SELECT id, name, slug FROM dishes WHERE slug LIKE 'what-to-serve-with-%' ORDER BY id;

-- Update all dish slugs to remove the "what-to-serve-with-" prefix
UPDATE dishes 
SET slug = REPLACE(slug, 'what-to-serve-with-', '')
WHERE slug LIKE 'what-to-serve-with-%';

-- Verify the update
SELECT id, name, slug FROM dishes ORDER BY id LIMIT 20;

-- Update image URLs for better food photos while we're at it
UPDATE dishes SET image_url = CASE
    -- Main dishes (without prefix now)
    WHEN slug = '15-bean-soup' THEN 'https://images.unsplash.com/photo-1547592166-23ac45744acd?w=800&h=600&fit=crop'
    WHEN slug = 'chicken-parmesan' THEN 'https://images.unsplash.com/photo-1632778149955-e80f8ceca2e8?w=800&h=600&fit=crop'
    WHEN slug = 'grilled-salmon' THEN 'https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=800&h=600&fit=crop'
    WHEN slug = 'beef-tacos' THEN 'https://images.unsplash.com/photo-1565299585323-38d6b0865b47?w=800&h=600&fit=crop'
    WHEN slug = 'vegetable-curry' THEN 'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=800&h=600&fit=crop'
    WHEN slug = 'beef-stew' THEN 'https://images.unsplash.com/photo-1547592166-23ac45744acd?w=800&h=600&fit=crop'
    WHEN slug = 'chicken-tikka-masala' THEN 'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=800&h=600&fit=crop'
    WHEN slug = 'spaghetti-carbonara' THEN 'https://images.unsplash.com/photo-1612874742237-6526221588e3?w=800&h=600&fit=crop'
    WHEN slug = 'fish-and-chips' THEN 'https://images.unsplash.com/photo-1579208030886-b937da0a5b4f?w=800&h=600&fit=crop'
    WHEN slug = 'vegetable-stir-fry' THEN 'https://images.unsplash.com/photo-1540809799-5da9372c3f64?w=800&h=600&fit=crop'
    ...
    -- Keep current image if not in the list
    ELSE image_url
END;